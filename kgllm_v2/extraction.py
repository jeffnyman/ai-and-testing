...
PROMPT_VARIANTS = {
  ...
  "tighter_predicates": SYSTEM_PROMPT_V2,
  "concept_seeded": SYSTEM_PROMPT_V3,
}
...
def call_ollama(passage: str, verbose: bool = True, system_prompt: str = None) -> dict:
...
def run_variants(passage: str, verbose: bool = False) -> dict:
  results = {}
  for name, prompt in PROMPT_VARIANTS.items():
    print(f"\n  Running variant: {name}")
    results[name] = call_ollama(passage, system_prompt=prompt, verbose=verbose)
  return results
